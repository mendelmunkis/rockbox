...
static void render_bookmarks(int off)
{
    ...
    y=1;
    fontheight = rb->font_get(rb->global_status->font_id[SCREEN_MAIN])->height;
    ...
    while (((y+fontheight) < LCD_HEIGHT) && cur.renderlen>0)
    {
        ...
        for (x=1,i=0,linedone=0; !linedone && i<LINEBUFLEN && cur.renderlen>0; )
        {
            ...
            default:
                ...
                {
                    /* display */
                    int charwidth=rb->font_get_width(rb->font_get(rb->global_status->font_id[SCREEN_MAIN]),ucs);
            ...
            case MARKUP_LINEFEED:
                ...
                if (!norender)
                {
                    ...
                    rb->lcd_putsxy(1+depth*rb->font_get_width(
                                       rb->font_get(rb->global_status->font_id[SCREEN_MAIN]),'_'),y,
...
loadnewarticle:
    ...
    while (!wrquit)
    {
        ...
        if (button & BUTTON_TOUCHSCREEN)
        {
            static int prevY = 0;
            short x, y;
            int fontheight = rb->font_get(rb->global_status->font_id[SCREEN_MAIN])->height;
